...
public final class LibUsb {
  ...
  /**
   * Debug and informational messages are printed to stdout, warnings and errors
   * to stderr.
   */
...
//  Capabilities supported by an instance of libusb on the current running  platform. Test if the loaded library supports a given capability by calling hasCapability.
  /**
   ...
   * Note that before being able to actually access an HID device, you may still
   * have to call additional libusb functions such as
...
//  Synchronization type for isochronous endpoints  .
  // Values for bits 2:3 of the bmAttributes field in EndpointDescriptor. Bits 3..2: Synchronization Type
  /**
...
//  Usage type for isochronous endpoints Values for bits 4:5 of the // bmAttributes field in EndpointDescriptor.
  /**
...
//  Transfer flags
   ...
   * (set to zero) here in the Java wrapper, since the ByteBuffer that acts as a
   * buffer for transfers is allocated by the JVM and is subject to garbage
   ...
   * <p>
   * If this flag is set, it is illegal to call {@link #freeTransfer(Transfer)}
   * from your transfer callback, as this will result in a double-free when this
   * flag is acted upon.
   */
   ...
   * <p>
   * This is useful when a device protocol mandates that each logical request is
   * terminated by an incomplete packet (i.e. the logical requests are not
   ...
   * endpoint's wMaxPacketSize. On transfers of other lengths, this flag has no
   * effect. Therefore, if you are working with a device that needs a ZLP
   * whenever the end of the logical request falls on a packet boundary, then it
   * is sensible to set this flag on every transfer (you do not have to worry
   * about only setting it on transfers that end on the boundary).
   * <p>
   * This flag is currently only supported on Linux. On other systems,
   * libusb_submit_transfer() will return {@link #ERROR_NOT_SUPPORTED} for every
   * transfer where this flag is set.
   */
  public static final byte TRANSFER_ADD_ZERO_PACKET = 8;

//  Transfer status codes
  /**
   * Transfer completed without error. Note that this does not indicate that the
   * entire amount of requested data was transferred.
   */
...
//  Hotplug events
   ...
   * responsibility to call {@link #close(DeviceHandle)} on any handle
   * associated with a disconnected device. It is safe to call
   * {@link #getDeviceDescriptor(Device, DeviceDescriptor)} on a device that has
   * left.
   */
...
//  private static final ConcurrentMap<Long, ImmutablePair<PollfdListener, Object>> pollfdListeners = new ConcurrentHashMap<Long, ImmutablePair<PollfdListener, Object>>();
  ...
  /**
   * Returns the API version of the underlying libusb library. It is defined as
   * follows: (major &lt;&lt; 24) | (minor &lt;&lt; 16) | (16 bit incremental)
   *
   ...
   * was initialized, this function does nothing: the message verbosity is fixed
   ...
   * {@link #freeDeviceList(DeviceList, boolean)} can unref all the devices for
   * you. Be careful not to unreference a device you are about to open until
   ...
   */
  public static native int getDeviceList(final Context context, final DeviceList list);
   ...
   *
   * @param device A device.
   * @param path   The array that should contain the port numbers. As per the
   *               USB 3.0 specs, the current maximum limit for the depth is 7.
   * @return The number of elements filled, {@link #ERROR_OVERFLOW} if the array
   *         is too small
   */
  public static native int getPortNumbers(final Device device, final ByteBuffer path);
   ...
   *         issuing {@link #freeDeviceList(DeviceList, boolean)}. The reason is
   *         that libusb currently does not maintain a permanent list of device
   *         instances, and therefore can only guarantee that parents are fully
   *         instantiated within a
   *         {@link #getDeviceList(Context, DeviceList)} - {@link #freeDeviceList(DeviceList, boolean)}
   *         block.
   */
   ...
   */
  public static native int getMaxPacketSize(final Device device, final byte endpoint);
   ...
   *         {@link #ERROR_NOT_FOUND} if the endpoint does not exist
   *         {@link #ERROR_OTHER} on other failure.
   */
  public static native int getMaxIsoPacketSize(final Device device, final byte endpoint);
   ...
   */
  public static native int getConfiguration(final DeviceHandle handle, final IntBuffer config);
   ...
   * {@link #releaseInterface(DeviceHandle, int)} first. You cannot change/reset
   ...
   *         configuration does not exist, {@link #ERROR_BUSY} if interfaces are
   *         currently claimed, {@link #ERROR_NO_DEVICE} if the device has been
   *         disconnected, another error code on other failure
   */
  public static native int setConfiguration(final DeviceHandle handle, final int config);
   ...
   */
  public static native int claimInterface(final DeviceHandle handle, final int iface);
   ...
   *         disconnected, another ERROR code on other failure
   */
  public static native int releaseInterface(final DeviceHandle handle, final int iface);
   ...
   *
   * @param handle           A device handle.
   * @param interfaceNumber  The bInterfaceNumber of the previously-claimed
   *                         interface
   * @param alternateSetting The bAlternateSetting of the alternate setting to
   *                         activate
   * @return 0 on success, {@link #ERROR_NOT_FOUND} if the interface was not
   ...
   */
  public static native int setInterfaceAltSetting(final DeviceHandle handle, final int interfaceNumber, final int alternateSetting);
   ...
   *         disconnected, another ERROR code on other failure.
   */
  public static native int clearHalt(final DeviceHandle handle, final byte endpoint);
   ...
   * rediscover the device. A return code of {@link #ERROR_NOT_FOUND} indicates
   ...
   *         required, or if the device has been disconnected another ERROR code
   *         on other failure
   */
   ...
   */
  public static native int kernelDriverActive(final DeviceHandle handle, final int interfaceNumber);
   ...
   * not detach it and return {@link #ERROR_NOT_FOUND}.
   *
   * @param handle          a device handle
   ...
   *         functionality is not available, another ERROR code on other failure
   *
   * @see #kernelDriverActive(DeviceHandle, int)
   */
  public static native int detachKernelDriver(final DeviceHandle handle, final int interfaceNumber);
   ...
   *         cannot be attached because the interface is claimed by a program or
   *         driver, anotherERROR code on other failure
   *
   * @see #kernelDriverActive(DeviceHandle, int)
   */
  public static native int attachKernelDriver(final DeviceHandle handle, final int interfaceNumber);
   ...
   */
  public static native int setAutoDetachKernelDriver(final DeviceHandle handle, final boolean enable);
   ...
   * code.
   *
   * @param errorCode The libusb error or libusb transfer status code to return
   *                  the name of.
   * @return The error name, or the string **UNKNOWN** if the value of errorCode
   *         is not a known error / status code.
   */
  ...
  /**
   * Returns the size in bytes of the buffer that's required to hold all of a
   * device descriptor's data.
   *
   ...
   */
  public static native int getStringDescriptorAscii(final DeviceHandle handle, final byte index, final StringBuffer string);

  /**
   * A simple wrapper around
   * {@link #getStringDescriptorAscii(DeviceHandle, byte, StringBuffer)}. It
   * simply returns the string (maximum length of 127) if possible. If not
   ...
   *
   * @param device     A device.
   * @param descriptor Output location for the USB configuration descriptor.
   *                   Only valid if 0 was returned. Must be freed with
   *                   {@link #freeConfigDescriptor(ConfigDescriptor)} after
   *                   use.
   ...
   *                   {@link #freeConfigDescriptor(ConfigDescriptor)} after
   *                   use.
   * @return 0 on success {@link #ERROR_NOT_FOUND} if the configuration does not
   *         exist another ERROR code on error.
   *
   * @see #getActiveConfigDescriptor(Device, ConfigDescriptor)
   * @see #getConfigDescriptorByValue(Device, byte, ConfigDescriptor)
   */
  public static native int getConfigDescriptor(final Device device, final byte index, final ConfigDescriptor descriptor);
   ...
   *                   use.
   * @return 0 on success {@link #ERROR_NOT_FOUND} if the configuration does not
   *         exist another ERROR code on error See also:
   *
   * @see #getActiveConfigDescriptor(Device, ConfigDescriptor)
   * @see #getConfigDescriptor(Device, byte, ConfigDescriptor)
   */
  public static native int getConfigDescriptorByValue(final Device device, final byte value, final ConfigDescriptor descriptor);
   ...
   *
   * @param context             The context to operate on, or NULL for the
   *                            default context.
   * @param endpointDescriptor  Endpoint descriptor from which to get the
   *                            superspeed endpoint companion descriptor.
   * @param companionDescriptor Output location for the superspeed endpoint
   *                            companion descriptor. Only valid if 0 was
   ...
   *
   * @param companionDescriptor The superspeed endpoint companion descriptor to
   *                            free
   */
   ...
   * @param descriptor Output location for the BOS descriptor. Only valid if 0
   *                   was returned. Must be freed with
   *                   {@link #freeBosDescriptor(BosDescriptor)} after use.
   * @return {@link #SUCCESS} on success, {@link #ERROR_NOT_FOUND} if the device
   *         doesn't have a BOS descriptor, another error code on error
   */
   ...
   * @param context             The context to operate on, or NULL for the
   *                            default context.
   * @param devCapDescriptor    Device Capability descriptor with a
   *                            bDevCapabilityType of
   *                            {@link #BT_USB_2_0_EXTENSION}.
   * @param extensionDescriptor Output location for the USB 2.0 Extension
   *                            descriptor. Only valid if 0 was returned. Must
   *                            be freed with
   *                            {@link #freeUsb20ExtensionDescriptor(Usb20ExtensionDescriptor)}
   *                            after use.
   * @return 0 on success a LIBUSB_ERROR code on error
   */
  public static native int getUsb20ExtensionDescriptor(final Context context, final BosDevCapabilityDescriptor devCapDescriptor, final Usb20ExtensionDescriptor extensionDescriptor);
   ...
   *
   * @param context                         The context to operate on, or NULL
   *                                        for the default context.
   * @param devCapDescriptor                Device Capability descriptor with a
   *                                        bDevCapabilityType of
   *                                        {@link #BT_SS_USB_DEVICE_CAPABILITY}.
   * @param ssUsbDeviceCapabilityDescriptor Output location for the SuperSpeed
   *                                        USB Device Capability descriptor.
   *                                        Only valid if {@link #SUCCESS} was
   *                                        returned. Must be freed with
   ...
   */
  public static native int getSsUsbDeviceCapabilityDescriptor(final Context context, final BosDevCapabilityDescriptor devCapDescriptor, final SsUsbDeviceCapabilityDescriptor ssUsbDeviceCapabilityDescriptor);
   ...
   */
  public static native void freeSsUsbDeviceCapabilityDescriptor(final SsUsbDeviceCapabilityDescriptor ssUsbDeviceCapabilityDescriptor);
   ...
   * @param containerIdDescriptor Output location for the Container ID
   *                              descriptor. Only valid if {@link #SUCCESS} was
   *                              returned. Must be freed with
   *                              {@link #freeContainerIdDescriptor(ContainerIdDescriptor)}
   *                              after use.
   * @return {@link #SUCCESS} on success or an error code on error
   */
  public static native int getContainerIdDescriptor(final Context context, final BosDevCapabilityDescriptor devCapDescriptor, final ContainerIdDescriptor containerIdDescriptor);
   ...
   *
   */
  public static int getDescriptor(final DeviceHandle handle, final EDescriptorType type, final byte index, final ByteBuffer data) {
//    return controlTransfer(handle,
//                           ENDPOINT_IN,  // bmRequestType
//                           REQUEST_GET_DESCRIPTOR, // bRequest
//                           (short) (((type & 0xff) << 8) | (index & 0xff)),  // wValue
//                           (short) 0, // wIndex
//                           data,  // data
//                           1000); // timeout
    return controlTransfer(handle,
...
//                           1000);
    return controlTransfer(handle,
                           ...
                           EDescriptorType.STRING.getWValue(index),// ((DT_STRING << 8) | (index & 0xff)),
                           langId,
                           data,
                           1000);
   ...
   *                      bmRequestType).
   * @param timeout       Timeout (in millseconds) that this function should
   ...
   *         {@link #ERROR_PIPE} if the control request was not supported by the
   ...
   * will return that data, so be sure to check the transferred output
   * parameter.
   * <p>
   ...
   * @return 0 on success (and populates transferred), {@link #ERROR_TIMEOUT} if
   *         the transfer timed out (and populates transferred),
   *         {@link #ERROR_PIPE} if the endpoint halted, {@link #ERROR_OVERFLOW}
   *         if the device offered more data, see Packets and overflows,
   *         {@link #ERROR_NO_DEVICE} if the device has been disconnected,
   ...
   *                    an unlimited timeout, use value 0.
   * @return 0 on success (and populates transferred), {@link #ERROR_TIMEOUT} if
   *         the transfer timed out, {@link #ERROR_PIPE} if the endpoint halted,
   ...
   * this locking.
   * <p>
   * While holding this lock, you are trusted to actually be handling events. If
   * you are no longer handling events, you must call
   ...
   * applications and asynchronous I/O. On the next iteration, your thread will
   * fail to obtain the events lock, and will hence become an event waiter.
   * <p>
   ...
   * so calling {@link #handleEvents(Context)} is not allowed.
   * <p>
   ...
   * to libusb's event handling loop functions (e.g.
   * {@link #handleEvents(Context)}) then you do not need to be concerned with
   * this locking.
   *
   * @param context The context to operate on, or NULL for the default context.
   ...
   * transfer has completed. Condition 3 is important because it means that the
   * thread that was previously handling events is no longer doing so, so if any
   * events are to complete, another thread needs to step up and start event
   * handling.
   * <p>
   ...
   * similar). You detect events on libusb's descriptors, so you then call this
   * function with a zero timeout value (while still holding the event lock).
   *
   * @param context The context to operate on, or NULL for the default context.
   * @param timeout The maximum time (In microseconds) to block waiting for
   ...
   * always return 0. This function allows you to detect whether you are running
   * on such a platform.
   *
   * @param context The context to operate on, or NULL for the default context
   ...
   * similar on libusb's file descriptors yourself - you do not need to use it
   * if you are calling {@link #handleEvents(Context)} or a variant directly.
   * <p>
   ...
   * {@link #handleEventsTimeout(Context, long)} (perhaps in non-blocking mode)
   * so that libusb can handle the timeout.
   * <p>
   * This function may return 1 (success) and an all-zero timeval. If this is
   * the case, it indicates that libusb has a timeout that has already expired
   * so you should call {@link #handleEventsTimeout(Context, long)} or similar
   ...
   *
   * @param context The context to operate on, or NULL for the default context
   * @param timeout Output location for a relative time against the current
   *                clock in which libusb must be called into in order to
   ...
   * {@link #exit(Context)} (e.g. when it is closing file descriptors that were
   * opened and added to the poll set at {@link #init(Context)} time). If you
   * don't want this, remove the notifiers immediately before calling
   * {@link #exit(Context)}.
   *
   * @param context  The context to operate on, or NULL for the default context.
  ...
  public static synchronized void setPollfdNotifiers(final Context context, final IPollfdListener listener, final Object userData) {
    ...
    if (listener == null) {
      ...
      pollfdListeners.put(contextId, new AbstractMap.SimpleEntry(listener, userData));
    }
  }
   ...
   * no longer needed, it should be freed with {@link #freeTransfer(Transfer)}.
   *
   ...
   * still required to call the {@link Transfer#setNumIsoPackets(int)} a
   ...
   * <p>
   * Please refer to {@link #TRANSFER_FREE_BUFFER} for an explanation of how
   * buffers are freed.
   * <p>
   ...
   * @return 0 on success, {@link #ERROR_NO_DEVICE} if the device has been
   *         disconnected, {@link #ERROR_BUSY} if the transfer has already been
   *         submitted. {@link #ERROR_NOT_SUPPORTED} if the transfer flags are
   *         not supported by the operating system. Another LIBUSB_ERROR code on
   *         failure.
   */
   ...
   * a transfer status of {@link #TRANSFER_CANCELLED}.
   *
   * @param transfer The transfer to cancel
   * @return 0 on success, {@link #ERROR_NOT_FOUND} if the transfer is already
   *         complete or cancelled. Another LIBUSB_ERROR code on failure.
   */
   ...
   * <p>
   * 3. If this is a host-to-device transfer with a data stage, put the data in
   * place after the setup packet.
   * <p>
   ...
   *                 first 8 bytes as a setup packet and infer the transfer
   *                 length from that.
  ...
  /**
   * Helper function to populate the required {@link Transfer} fields for a bulk
   * transfer.
   *
   * @param transfer The transfer to populate.
   * @param handle   Handle of the device that will handle the transfer.
   ...
   * @param transfer      The transfer to populate.
   * @param handle        Handle of the device that will handle the transfer.
   * @param endpoint      Address of the endpoint where this transfer will be
   *                      sent.
   * @param buffer        Data buffer.
   * @param numIsoPackets The number of isochronous packets.
   * @param callback      Callback function to be invoked on transfer
   *                      completion.
   * @param userData      User data to pass to callback function.
   * @param timeout       Timeout for the transfer in milliseconds.
   */
   ...
   * Typically you will assign equal lengths to each packet in the transfer, and
   * hence the above method is sub-optimal. You may wish to use
  ...
  /**
   * Convenience function to locate the position of an isochronous packet within
   * the buffer of an isochronous transfer, for transfers where each packet is
   * of identical size.
   * <p>
   ...
   * of the packet buffer is then just a simple calculation: buffer +
   * (packet_size * packet)
   * <p>
   ...
   * <p>
   * Register a callback with the {@link Context}. The callback will fire when a
   * matching event occurs on a matching device. The callback is armed until
   * either it is deregistered with
   * {@link #hotplugDeregisterCallback(Context, HotplugCallbackHandle)} or the
   * supplied callback returns 1 to indicate it is finished processing events.
   *
   ...
   * @param productId      The product id to match or
   *                       {@link #HOTPLUG_MATCH_ANY}.
   * @param deviceClass    The device class to match or
   *                       {@link #HOTPLUG_MATCH_ANY}.
   ...
   * @param callbackHandle Hotplug callback handle of the allocated callback.
   *                       Only needed if you later want to deregister this
   *                       callback, can be NULL.
   * @return {@link #SUCCESS} on success, some ERROR code on failure.
   */
  @SuppressWarnings("unchecked")
  public static synchronized int hotplugRegisterCallback(final Context context,
                                                         final int events,
                                                         final int flags,
                                                         final int vendorId,
                                                         final int productId,
                                                         final int deviceClass,
                                                         final IHotplugCallback callback,
                                                         final Object userData,
...
//    hotplugCallbacks.put(globalHotplugId, new ImmutablePair<>(callback, userData));
   ...
   * <p>
   * Deregister a callback from a {@link Context}. This function is safe to call
  ...
  public static void hotplugDeregisterCallback(final Context context, final HotplugCallbackHandle callbackHandle) {
    ...
    /**
     * When a handle is assigned by a register call, its value is the same as
     * the one of globalHotplugId at that moment, which is what's used to
     * identify data in the hotplugCallbacks map.
     * <p>
     * This is because globalHotplugId pretty much mirrors the behavior of the
     * handle: integer starting at 1, incremented each time by one. Problems
     * could arise from concurrency, but are completely avoided by fully
     * serializing register calls.
     * <p>
     * As such, we can use the handle value from callbackHandle to correctly
     * remove the data from the hotplugCallbacks map.
     */
