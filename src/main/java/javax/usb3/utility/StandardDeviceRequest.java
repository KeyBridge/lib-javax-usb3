...
import javax.usb3.enumerated.EFeatureSelector;
...
public class StandardDeviceRequest {
   ...
   * <p>
   * <p>
   * Valid recipients are {@link ERecipient#DEVICE device},
   * {@link ERecipient#INTERFACE interface}, and
   * {@link ERecipient#ENDPOINT endpoint}. If the recipient is device, the
   * target must be 0. If the recipient is interface or endpoint, the target is
   * the interface number or endpoint address, respectively.
   *
   ...
   * table 9.5, which includes {@link javax.usb.enumerated.EDescriptorType#DEVICE device},
   * {@link javax.usb.enumerated.EDescriptorType#CONFIGURATION configuration},
   * and {@link javax.usb.enumerated.EDescriptorType#STRING string} descriptor
   * types. Note that devices normally do not support requests for interface or
   ...
   * <p>
   * If the type is string, the langid indicates what language to use. For other
   * types it should be 0 (but thi s is not enforced).
   * <p>
   ...
   * {@link ERecipient#INTERFACE interface}, and
   * {@link ERecipient#ENDPOINT endpoint}. If the recipient is device, the
   * target is either 0 or the Test Selector. If the recipient is interface or
   ...
   * Valid recipients are {@link ERecipient#DEVICE device},
   * {@link ERecipient#INTERFACE interface}, and
   * {@link ERecipient#ENDPOINT endpoint}.
   *
   * @param recipient The recipient.
   * @exception IllegalArgumentException If the recipient is not valid.
   */
  private static void checkRecipient(ERecipient recipient) throws IllegalArgumentException {
    switch (recipient) {
      case DEVICE:
      case INTERFACE:
      case ENDPOINT:
