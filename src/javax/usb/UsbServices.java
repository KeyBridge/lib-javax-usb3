...
public interface UsbServices {
  ...
  /**
   * Get the virtual UsbHub to which all physical Host Controller UsbHubs are
   * attached.
   * <p>
   ...
   * physical Host Controller's emulated root hub as the root UsbHub of that
   ...
   * UsbHub already has a list of UsbDevices, and a UsbHub <i>is</i> a
   ...
   * configuration involves getting the first root UsbHub, getting all its
   * attached UsbDevices, and checking each device; any of those devices which
   * are UsbHubs can be also searched recursively. Then, the entire operation
   * must be performed on the next root UsbHub, and this is repeated for all the
   * root UsbHubs in the array/list. In the second configuration, the virtual
   * root UsbHub is recursively searched in a single operation.
   * <p>
   * The second configuration is what is used in this API. The implementation is
   * responsible for creating a single root UsbHub which is completely virtual
   * (and available through the UsbServices object). Every UsbHub attached to
   * this virtual root UsbHub corresponds to a physical Host Controller's
   * emulated hub. I.e., the first level of UsbDevices under the virtual root
   * UsbHub are all UsbHubs corresponding to a particular Host Controller on the
   * system. Note that since the root UsbHub is a virtual hub, the number of
   * ports is not posible to specify; so all that is guaranteed is the number of
   * ports is at least equal to the number of UsbHubs attached to the root
   * UsbHub. The number of ports on the virtual root UsbHub may change if
   * UsbHubs are attached or detached (e.g., if a Host Controller is physically
   ...
   * ports for the root UsbHub equal the number of directly attached UsbHubs.
   * <p>
  ...
  /**
   * Get the (minimum) version number of the javax.usb API that this UsbServices
   * implements.
   * <p>
